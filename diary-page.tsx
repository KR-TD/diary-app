                ...
                {diaryEntries.length === 0 ? (
                  ...
                  <>
                    ...
                    <Pagination className="mt-4">
                      <PaginationContent>
                        {(() => {
                          const groupSize = 5; // 한 번에 보여줄 숫자 개수(5 고정)
                          const groupStart = Math.floor((currentPage - 1) / groupSize) * groupSize + 1; // 1, 6, 11 ...
                          const groupEnd = Math.min(groupStart + groupSize - 1, totalPages);

                          const goFirst = (e: React.MouseEvent) => {
                            e.preventDefault();
                            if (currentPage !== 1) setCurrentPage(1); // << 맨 처음
                          };

                          const goPrev = (e: React.MouseEvent) => {
                            e.preventDefault();
                            if (currentPage > 1) setCurrentPage(currentPage - 1); // < 이전 페이지
                          };

                          const goNext = (e: React.MouseEvent) => {
                            e.preventDefault();
                            if (currentPage < totalPages) setCurrentPage(currentPage + 1); // > 다음 페이지
                          };

                          const goLast = (e: React.MouseEvent) => {
                            e.preventDefault();
                            if (currentPage !== totalPages && totalPages > 0) setCurrentPage(totalPages); // >> 맨 끝
                          };
                          ...
                          return (
                            <>
                              ...
                              {/* 숫자 5개 고정 윈도우 (현재 페이지에 따라 자동으로 1~5, 6~10 ... 윈도우 이동) */}
